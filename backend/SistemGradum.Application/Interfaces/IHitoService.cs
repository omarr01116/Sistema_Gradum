...
public interface IHitoService
{
    ...
    Task<(bool Success, string? Error)> CompletarAsync(int id, int? documentoEvidenciaId, int usuarioId, int? asesorIdFiltro);
    ...
    Task<int?> ObtenerProyectoIdAsync(int hitoId);
}